...
| `notes/type=*/database=*/schema=*/<table>.md`   | Human-written per-table notes (added with `nao notes <connection> <schema> <table>`). Never overwritten/deleted by sync, and folded into that table's `how_to_use.md` on the next sync.                    | If a table has notes, they already surface inside `how_to_use.md` — no need to duplicate them in `RULES.md`.                                                          |
