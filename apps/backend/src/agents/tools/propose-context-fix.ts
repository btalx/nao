...
export function createContextFixCollector(
	...
	const editTool = createTool<EditInput, Ack>({
		description:
			...
			'notes/**, nao_config.yaml, agent/**) or a linked GitHub repo file under repos/<name>/** to fix a ' +
			'recommendation you just recorded. Prefer notes/type=<type>/database=<name>/schema=<schema>/<table>.md ' +
			"for a fix about one specific table — it is folded into that table's how_to_use.md on the next sync. " +
			'Never target generated warehouse files (databases/**) or unlinked repos/** paths — use ' +
	...
	if (!linkedRepo) {
		throw new Error(
			`"${filePath}" is auto-generated by \`nao sync\` and would be overwritten. ` +
				'Encode the fix in a human-written file (RULES.md, semantics/**, or notes/** for a single table) ' +
				'or call propose_manual_fix instead.',
		);
	}
